...
function handleSaveRoute() {
  ...
  // FILTER -----------------------------------------------------------------
  const cleanedCafes = cafes
    .map((cafe) => ({
      ...cafe,
      bikes: cafe.bikes === true,
      scooters: cafe.scooters === true,
      cyclists: cafe.cyclists === true,
      cars: cafe.cars === true,
      offRoadParking: cafe.offRoadParking === true,
      pets: cafe.pets === true,
      disability: cafe.disability === true,
      evcharger: cafe.evcharger === true,
    }))
    .filter(
      (cafe) =>
        cafe.coords &&
        typeof cafe.coords.latitude === "number" &&
        typeof cafe.coords.longitude === "number" &&
        !Number.isNaN(cafe.coords.latitude) &&
        !Number.isNaN(cafe.coords.longitude)
    );

  const filteredCafes = cleanedCafes.filter((cafe) => {
    const ratingMatch =
      cafe.avgOverall >= filter.overall &&
      cafe.avgService >= filter.service &&
      cafe.avgValue >= filter.value;

    const selectedFacilities = Object.keys(facilityFilters).filter(
      (key) => facilityFilters[key]
    );
    const facilityMatch = selectedFacilities.every(
      (key) => cafe[key] === true
    );
...
>
        ...
        {Array.isArray(filteredCafes) && filteredCafes.length === 0 && (
          ...
          >
            ...
            >
              <Text style={{ color: "#fff" }}>No cafés match the selected filters.</Text>
            </View>
          </View>
        )}
...
<TouchableOpacity
  ...
  onPress={async () => {
    ...
    try {
      // --- Snap route before saving ---
      const rawCoords = [routeData.start, ...(waypoints || []), routeData.end];
      const path = rawCoords.map(p => `${p.latitude},${p.longitude}`).join("|");
      ...
      if (data.snappedPoints?.length) {
        routeData.snappedCoords = data.snappedPoints.map(p => ({
          latitude: p.location.latitude,
          longitude: p.location.longitude,
        }));
      } else {
        console.warn("No snapped points returned, saving raw route instead");
        routeData.snappedCoords = rawCoords;
      }
      ...
      console.error("Failed to save snapped route:", err);
    }
  }}
>
