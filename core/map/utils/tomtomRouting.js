...
const tomtomApiKey = Constants.expoConfig?.extra?.tomtomApiKey;
  ...
  try {
    ...
    // Strategy: Each step contains the instruction for what comes AFTER it (the next maneuver)
    // This way we always display the correct "next instruction" without ambiguity
    const steps = instructions.length > 0 ? instructions.map((instr, idx) => {
      ...
      if (maneuver.includes("ROUNDABOUT")) {
        ...
        if (!exitNumber) {
          ...
          if (roundaboutMatch) {
            ...
            console.log('[tomtomRouting] Roundabout: Extracted from maneuver code:', exitNumber);
          }
        }
      ...
      // Extract next instruction (what to do after this step)
    ...
    // Log ALL instructions - raw AND processed - for debugging
    instructions.forEach((instr, idx) => {
      const step = steps[idx];
      console.log(`[tomtomRouting] INSTRUCTION ${idx}:`, {
        rawText: instr.text,
        rawManeuver: instr.maneuver,
        rawRoundaboutExitNumber: instr.roundaboutExitNumber,
        processedInstruction: step?.instruction,
        processedManeuver: step?.maneuver,
        processedExitNumber: step?.roundaboutExitNumber,
        street: instr.street,
